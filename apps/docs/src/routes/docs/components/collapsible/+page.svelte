<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'open', type: 'boolean', default: 'false', description: 'Bindable open state.' },
		{ component: 'Root', prop: 'disabled', type: 'boolean', default: 'false', description: 'Disable the trigger.' },
		{ component: 'Trigger', prop: '--', type: '--', default: '--', description: 'Toggles `open`. Renders as a `<button>` you can style.' },
		{ component: 'Content', prop: '--', type: '--', default: '--', description: 'Auto-animated via CSS grid-template-rows -- no height measurement.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="Show or hide content with a smooth height transition." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A single show/hide region. The expand/collapse animation runs on CSS grid-template-rows — no JS height measurement.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[12rem] items-center justify-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<Collapsible.Root bind:open>
					<div class="w-80 rounded-[var(--radius-md)] border border-border bg-background">
						<Collapsible.Trigger class="flex w-full items-center justify-between px-4 py-2.5 text-[0.86rem] [font-weight:var(--font-weight-label,500)] [letter-spacing:var(--tracking-label,0em)] text-foreground">
							@aidan-neel starred 3 repos
							<ChevronDown size={14} class={`transition-transform ${open ? 'rotate-180' : ''}`} />
						</Collapsible.Trigger>
						<Collapsible.Content>
							<div class="flex flex-col gap-1.5 border-t border-border px-4 py-3 text-[0.82rem] text-foreground-muted">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Collapsible.Root bind:open>
  <Collapsible.Trigger>Show details</Collapsible.Trigger>
  <Collapsible.Content>
    {/* …content… */}
  </Collapsible.Content>
</Collapsible.Root>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Collapsible.{row.component}</code><code class="font-mono text-[0.82rem] [font-weight:var(--font-weight-label,600)] [letter-spacing:var(--tracking-label,0em)]">{row.prop}</code></div>
