<script lang="ts">
	...
	const apiRows = [
		{ prop: 'for', type: 'string', default: '--', description: 'Standard HTML `for` attribute targeting the input id.' },
		{ prop: '...HTMLLabelAttributes', type: '--', default: '--', description: 'Spread onto the underlying <label>.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A form field label, accessible by default." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A theme-aware form label that pairs with any input, fades correctly when the field is disabled via <code class="font-mono text-[0.82rem]">peer-disabled</code>.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Label for="email">Email address</Label>
<Input id="email" type="email" />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
