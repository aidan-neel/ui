<script lang="ts">
	...
	const apiRows = [
		{ prop: 'pressed', type: 'boolean', default: 'false', description: 'Bindable. Current pressed state.' },
		{ prop: 'disabled', type: 'boolean', default: 'false', description: 'Prevent toggling and dim the visuals.' },
		{ prop: 'size', type: '"sm" | "md" | "lg"', default: '"md"', description: 'Hit area preset.' },
		{ prop: 'variant', type: '"default" | "outlined"', default: '"default"', description: 'Outlined adds a border.' },
		{ prop: 'onPressedChange', type: '(pressed) => void', default: '--', description: 'Fires after `pressed` changes.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A two-state button — pressed or unpressed." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A pressable button with an explicit on/off state. Pair multiples in a Toggle Group for radio-like or multi-select behavior.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] items-center justify-center gap-2 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Toggle bind:pressed={bold} aria-label="Bold">
  <Bold size={14} />
</Toggle>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
