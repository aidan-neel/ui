<script lang="ts">
	...
	const apiRows = [
		{ prop: 'orientation', type: '"vertical" | "horizontal" | "both"', default: '"vertical"', description: 'Which axis can scroll.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A scroll container with a themed scrollbar." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A scroll container that styles its scrollbar to match the theme. Pure CSS, no shadow DOM, no measurement loops.</p>
	</div>
				...
				<ScrollArea class="h-48 w-64 rounded-[var(--radius-md)] border border-border bg-background">
					<div class="flex flex-col">
						{#each items as item, i}
							<div class="border-b border-border/60 px-3 py-2 text-[0.86rem] text-foreground" class:border-b-0={i === items.length - 1}>{item}</div>
						{/each}
					</div>
				</ScrollArea>
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<ScrollArea class="h-48 w-64">
  {#each items as item}
    <div>{item}</div>
  {/each}
</ScrollArea>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">ScrollArea</code><code class="font-mono text-[0.82rem] [font-weight:var(--font-weight-label,600)] [letter-spacing:var(--tracking-label,0em)]">{row.prop}</code></div>
