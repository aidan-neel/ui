<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'openDelay', type: 'number (ms)', default: '200', description: 'Time the cursor must rest on the trigger before opening.' },
		{ component: 'Root', prop: 'closeDelay', type: 'number (ms)', default: '150', description: 'Time the cursor must leave before closing.' },
		{ component: 'Trigger', prop: 'href', type: 'string', default: '--', description: 'Renders as an anchor when set; falls back to a focusable span.' },
		{ component: 'Content', prop: 'side / align', type: 'placement', default: '"bottom" / "center"', description: 'Floating-UI placement.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A floating preview that opens on hover." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A preview card that opens on hover or keyboard focus. Use it for user mentions, link previews, or definitions.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<HoverCard.Root>
  <HoverCard.Trigger href="/profile">@aidan-neel</HoverCard.Trigger>
  <HoverCard.Content>
    <HoverCard.Title>Aidan Neel</HoverCard.Title>
    <HoverCard.Description>Building Silk.</HoverCard.Description>
  </HoverCard.Content>
</HoverCard.Root>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">HoverCard.{row.component}</code><code class="font-mono text-[0.82rem] [font-weight:var(--font-weight-label,600)] [letter-spacing:var(--tracking-label,0em)]">{row.prop}</code></div>
