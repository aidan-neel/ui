<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'value', type: 'string', default: '--', description: 'Bindable selected value.' },
		{ component: 'Root', prop: 'name', type: 'string', default: '--', description: 'Form field name for the underlying `<input>`s.' },
		{ component: 'Root', prop: 'disabled', type: 'boolean', default: 'false', description: 'Disable the entire group.' },
		{ component: 'Item', prop: 'value', type: 'string', default: '--', description: 'Identifier matched against the group value.' },
		{ component: 'Item', prop: 'label', type: 'string', default: '--', description: 'Visible label rendered next to the radio.' },
		{ component: 'Item', prop: 'description', type: 'string', default: '--', description: 'Optional secondary line beneath the label.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A group of mutually-exclusive options." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A themed wrapper around the native radio input. Works inside any form with no extra wiring.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<RadioGroup.Root bind:value name="plan">
  <RadioGroup.Item value="free" label="Free" />
  <RadioGroup.Item value="pro" label="Pro" />
</RadioGroup.Root>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">RadioGroup.{row.component}</code><code class="font-mono text-[0.82rem] [font-weight:var(--font-weight-label,600)] [letter-spacing:var(--tracking-label,0em)]">{row.prop}</code></div>
