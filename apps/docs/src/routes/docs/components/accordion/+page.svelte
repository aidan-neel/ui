<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'type', type: '"single" | "multiple"', default: '"single"', description: 'Single allows one panel open at a time; multiple allows any combination.' },
		{ component: 'Root', prop: 'value', type: 'string | string[]', default: '--', description: 'Bindable open item value(s).' },
		{ component: 'Root', prop: 'collapsible', type: 'boolean', default: 'true', description: 'In single mode, allow closing the active panel.' },
		{ component: 'Item', prop: 'value', type: 'string', default: '--', description: 'Identifier for the item.' },
		{ component: 'Item', prop: 'disabled', type: 'boolean', default: 'false', description: 'Disable this item.' },
		{ component: 'Trigger / Content', prop: '--', type: '--', default: '--', description: 'Header button and the revealed region.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="Stacked collapsible sections — single or multi-open." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">Stacked Collapsibles with shared state. Single mode acts like a radio; multiple lets users open any combination.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[14rem] items-center justify-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<div class="w-full max-w-md">
					<Accordion.Root type="single" value="item-1">
						<Accordion.Item value="item-1">
							<Accordion.Trigger>Is it accessible?</Accordion.Trigger>
							<Accordion.Content>Yes — the trigger is a real button with `aria-expanded` and `aria-controls`, and the content has `role="region"`.</Accordion.Content>
						</Accordion.Item>
						<Accordion.Item value="item-2">
							<Accordion.Trigger>Does it animate?</Accordion.Trigger>
							<Accordion.Content>Yes — height is animated via CSS grid-template-rows. No JS measurement, no jank.</Accordion.Content>
						</Accordion.Item>
						<Accordion.Item value="item-3">
							<Accordion.Trigger>Does it theme?</Accordion.Trigger>
							<Accordion.Content>Yes — open the theme studio and watch the chevron, duration, and colors update live.</Accordion.Content>
						</Accordion.Item>
					</Accordion.Root>
				</div>
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Accordion.Root type="single">
  <Accordion.Item value="a">
    <Accordion.Trigger>Section A</Accordion.Trigger>
    <Accordion.Content>…</Accordion.Content>
  </Accordion.Item>
</Accordion.Root>`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Accordion.{row.component}</code><code class="font-mono text-[0.82rem] [font-weight:var(--font-weight-label,600)] [letter-spacing:var(--tracking-label,0em)]">{row.prop}</code></div>
