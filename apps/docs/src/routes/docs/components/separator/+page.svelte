<script lang="ts">
	...
	const apiRows = [
		{ prop: 'orientation', type: '"horizontal" | "vertical"', default: '"horizontal"', description: 'Axis the rule runs along.' },
		{ prop: 'decorative', type: 'boolean', default: 'true', description: 'When false, the divider is announced to assistive tech as a real separator.' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			A 1px rule for splitting groups of content. Defaults to horizontal but flips to vertical with one prop.
		</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] flex-col items-center justify-center gap-4 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				...
				<div class="flex h-6 items-center gap-3 text-[0.78rem]">
					<span>Docs</span>
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Separator />
<Separator orientation="vertical" />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
