<script lang="ts">
	...
	const apiRows = [
		...
		{ prop: 'weekStartsOn', type: '0 - 6', default: '0', description: '0 = Sunday, 1 = Monday, ...' },
		{ prop: 'onValueChange', type: '(date) => void', default: '--', description: 'Fires after the user picks a date.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A single-date calendar with locale-aware formatting." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A single-date calendar. Month names and weekday letters use the user's locale via <code class="font-mono text-[0.82rem]">Intl</code> — no date library required.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[18rem] items-center justify-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<Calendar bind:value={date} weekStartsOn={1} />
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Calendar bind:value={date} weekStartsOn={1} />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
