<script lang="ts">
	...
	const apiRows = [
		{ prop: 'page', type: 'number', default: '1', description: 'Bindable current page (1-indexed).' },
		{ prop: 'total', type: 'number', default: '--', description: 'Total page count.' },
		{ prop: 'siblings', type: 'number', default: '1', description: 'Adjacent pages shown either side of `page` before truncating with an ellipsis.' },
		{ prop: 'onPageChange', type: '(page) => void', default: '--', description: 'Fires after navigation.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A compact page navigator with ellipsis truncation." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A compact pager that truncates with an ellipsis. Always shows the first and last page so users know how big the data set is.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Pagination bind:page total={20} />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
