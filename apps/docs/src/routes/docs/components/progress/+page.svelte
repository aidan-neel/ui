<script lang="ts">
	...
	const apiRows = [
		{ prop: 'value', type: 'number', default: '0', description: 'Current progress, clamped between 0 and `max`.' },
		{ prop: 'max', type: 'number', default: '100', description: 'Upper bound.' },
		{ prop: 'indeterminate', type: 'boolean', default: 'false', description: 'Switch to a continuous looping animation when the duration is unknown.' }
	];

	let val = $state(28);
	onMount(() => {
		const id = setInterval(() => { val = (val + 4) % 100; }, 600);
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="Determinate and indeterminate progress bars." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">Show how far along a task is. Skip the value to show a looping indicator for tasks of unknown duration.</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] flex-col items-center justify-center gap-6 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<div class="flex w-full max-w-md flex-col gap-2">
					<div class="flex items-center justify-between text-[0.78rem] text-foreground-muted"><span>Uploading…</span><span>{val}%</span></div>
					<Progress value={val} />
				</div>
				<div class="flex w-full max-w-md flex-col gap-2">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Progress value={28} />
<Progress indeterminate />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
