	...
	const apiRows = [
		{
			...
			type: 'string',
			default: '--',
			description: 'Key combination to render and listen for (e.g. `"K"`, `"shift+/"`).'
		},
		{
			prop: 'children',
			type: 'Snippet',
...
<section class="pt-10">
	<div class="relative">
		...
		>
			<div
				class="flex min-h-[10rem] flex-wrap items-center justify-center gap-4 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8"
			>
