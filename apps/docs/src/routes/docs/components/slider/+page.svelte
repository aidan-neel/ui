<script lang="ts">
	...
	const apiRows = [
		...
		{ prop: 'step', type: 'number', default: '1', description: 'Increment per keypress / drag step.' },
		{ prop: 'disabled', type: 'boolean', default: 'false', description: 'Disable the control.' },
		{ prop: 'label', type: 'string', default: '--', description: 'Accessible name (set via `aria-label`).' },
		{ prop: 'onValueChange', type: '(value) => void', default: '--', description: 'Fires after each value change.' }
	];
...
<svelte:head><title>Silk · {TITLE}</title><meta name="description" content="A native range input dressed in the theme." /></svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		...
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">A themed wrapper around the native range input. Keyboard-driven for free, drag-to-set on every device.</p>
	</div>
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] items-center justify-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<div class="flex w-full max-w-sm flex-col gap-2">
					<div class="flex items-center justify-between text-[0.78rem]"><span class="text-foreground-muted">Volume</span><span class="font-mono text-foreground">{volume}</span></div>
					<Slider bind:value={volume} label="Volume" />
				</div>
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Slider bind:value={volume} label="Volume" />`, 'svelte')}</code></pre>
		</div>
	</div>
</section>
