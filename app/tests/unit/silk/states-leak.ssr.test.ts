...
import { states, useState } from '../../../src/lib/silk/internals/state.svelte.ts';
import {
	toast,
	__setActiveToastStateForTests,
	__getActiveToastStateForTests
} from '../../../src/lib/silk/components/toast/lib.svelte.ts';
import { UIState } from '../../../src/lib/silk/internals/state.svelte.ts';
import type { ToastUIState } from '../../../src/lib/silk/components/toast/lib.svelte.ts';

import ModalFixture from '../../fixtures/ModalFixture.svelte';
import SheetFixture from '../../fixtures/SheetFixture.svelte';

/*
 * Multi-request states-registry leak test — strategy §6 SSR tier,
 * failure modes (i) and (ii).
 *
 * BACKGROUND (audit §7 SSR tier, pattern guide §14.2)
 * silk's global registry at `internals/state.svelte.ts:30` is
 * `$state<Record<string, UIState<...>>>({})` — module-scoped.
 *
 * SPRINT 5 EMPIRICAL RESULT (pre-Phase 4)
 ...
 *        renders (P3-F12).
 *
 * PHASE 4 ITEM 1 FIX (this state)
 * Toast state is now per-Toaster-mount. The free-function `toast(...)` API
 * is a no-op when no Toaster has mounted (i.e., during SSR). This test
 * file's assertions flip:
...
describe('Failure mode (ii) — toast cross-request bleed (FIXED in Phase 4 item 1)', () => {
