...
/*
 * A11y tier — strategy §14.1.
 *
 * SCOPE: axe-core WCAG rule subset only. Per strategy §1.1, axe does NOT
 ...
 *
 * The Tier 1 component list comes from pattern guide §14.1.
 *
...
async function runAxe(): Promise<{
	...
	const result = await axe.run(document.body, {
		rules: {
			...
			// color-contrast — deferred to a separate Phase 4 theme-tokens pass
			// per Sprint 5 / item 2 direction. The aria fixes below pass; the
			// theme-contrast suite at the bottom of this file is the
			// it.skip'd location that re-enables this rule when the theme
			// fixes land.
 ...
 * Strategy §1.1 names these as Playwright responsibility.
 */
...
describe('Keyboard nav — calendar (P3-F10 fixed)', () => {
	it('arrow keys move focus one day at a time', async () => {
		const { default: Calendar } = await import(
			'../../../src/lib/silk/components/calendar/calendar.svelte'
		);
 ...
 *
 * Phase 4 item 2 fixed all the aria violations on popover/dropdown-menu/
 * select/combobox/sheet. The remaining color-contrast violations are
 * theme-token issues that need a separate pass to adjust the default
 * theme's panel/menu-item color tokens against WCAG 2 AA contrast
 * thresholds.
 *
