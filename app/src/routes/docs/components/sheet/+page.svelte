<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'open', type: 'boolean', default: 'false', description: 'Bindable controlled open state.' },
		{ component: 'Trigger', prop: '…ButtonProps', type: '—', default: '—', description: 'Renders as a Button.' },
		{ component: 'Content', prop: 'side', type: '"left" | "right"', default: '"right"', description: 'Edge to anchor to. Animation flips automatically.' },
		{ component: 'Content', prop: 'allowClickOutside', type: 'boolean', default: 'true', description: 'Disable to force explicit close.' },
		{ component: 'Title / Description / Header / Footer', prop: '—', type: '—', default: '—', description: 'Layout slots — same shape as Dialog.' },
		{ component: 'Close', prop: '—', type: '—', default: '—', description: 'Closes the sheet on click.' }
	];
...
<svelte:head>
	...
	<meta name="description" content="An edge-anchored drawer for mobile menus, filters, and side panels." />
</svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Sheet</h1>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] items-center justify-center gap-3 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<Sheet.Root>
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Sheet.Root>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Sheet.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
