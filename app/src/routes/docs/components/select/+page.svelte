<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'value', type: 'string', default: '""', description: 'Bindable. Two-way sync with item value.' },
		{ component: 'Trigger', prop: '…ButtonProps', type: '—', default: '—', description: 'Renders as a Button. Children become the display label.' },
		{ component: 'Content', prop: 'children', type: 'Snippet', default: '—', description: 'Floating panel — anchored below the trigger.' },
		{ component: 'Item', prop: 'value', type: 'string', default: '—', description: 'Stable identifier passed back to Root.value.' },
		{ component: 'Item', prop: 'onclick', type: '() => void', default: '—', description: 'Fires when picked. Update bound state here.' },
		{ component: 'Label', prop: 'children', type: 'Snippet', default: '—', description: 'Non-interactive group heading.' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Select</h1>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Select.Root value={role}>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Select.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
