<script lang="ts">
	...
	const apiRows = [
		{ prop: 'variant', type: '"primary" | "secondary" | "outlined" | "flat" | "ghost" | "alternate" | "destructive"', default: '"primary"', description: 'Visual treatment.' },
		{ prop: 'href', type: 'string', default: '—', description: 'Renders as an `<a>` instead of a `<span>`.' },
		{ prop: 'class', type: 'string', default: '—', description: 'Tailwind classes appended via cn().' },
		{ prop: 'children', type: 'Snippet', default: '—', description: 'Label content (icons supported).' }
	];

	const playgroundCode = $derived(`<Badge${pgVariant !== 'primary' ? ` variant="${pgVariant}"` : ''}>${pgLabel || 'Badge'}</Badge>`);
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">
			Badge
		</h1>
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			A compact label for status, counts, tags, and metadata. Same variant grammar as Button so
	...
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<div class="flex flex-col divide-y divide-border/60">
				...
				<div class="flex flex-col gap-2 px-6 py-4">
					<span class="text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted">Variant</span>
...
<div class="flex flex-col gap-16 pt-16">
	...
	<section class="scroll-mt-20 flex flex-col gap-5">
		...
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
			{#each variantList as v}
				<div class="group flex flex-col gap-2 rounded-[var(--radius-lg)] border border-border bg-card p-4">
					...
					<button
						type="button"
						onclick={() => copy(`<Badge${v.value !== 'primary' ? ` variant="${v.value}"` : ''}>${v.label}</Badge>`, `var-${v.value}`)}
	...
	<section class="scroll-mt-20 flex flex-col gap-5">
		...
		<div class="grid grid-cols-1 gap-3 md:grid-cols-3">
			<div class="flex flex-col items-start gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-4">
				<span class="text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted">Status pill</span>
				<Badge variant="flat">
			...
			<div class="flex flex-col items-start gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-4">
				<span class="text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted">Count</span>
				<Badge variant="ghost" class="px-1.5 text-[0.66rem]">12 new</Badge>
			</div>
			<div class="flex flex-col items-start gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-4">
				<span class="text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted">Tag</span>
