<script lang="ts">
	...
	const apiRows = [
		...
		{ prop: 'description', type: 'string', default: '—', description: 'Helper text below the field.' },
		{ prop: 'placeholder', type: 'string', default: '—', description: 'Standard HTML placeholder.' },
		{ prop: 'rows', type: 'number', default: '4', description: 'Initial visible row count — height grows from there.' },
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Textarea</h1>
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="grid min-h-[14rem] place-items-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<div class="w-full max-w-[28rem]">
					<Textarea label="Description" placeholder="Tell us about your project…" bind:value={pgValue} />
				</div>
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Textarea
  label="Description"
