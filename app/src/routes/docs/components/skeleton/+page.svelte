<script lang="ts">
	...
	const apiRows = [
		...
		{ prop: 'unit', type: '"px" | "rem" | "%" | "vh" | …', default: '"px"', description: 'CSS unit for the dimensions.' },
		{ prop: 'class', type: 'string', default: '—', description: 'Tailwind classes for shape — e.g. `rounded-full`.' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Skeleton</h1>
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			A neutral placeholder rectangle for content that's still loading. Compose any shape by
			passing Tailwind sizing classes — Skeleton itself has no opinions about dimensions.
		</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<div class="flex items-center gap-4">
