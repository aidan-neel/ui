<script lang="ts">
	...
	const apiRows = [
		{ prop: 'shortcut', type: 'string', default: '—', description: 'Key combination to render and listen for (e.g. `"K"`, `"shift+/"`).' },
		{ prop: 'children', type: 'Snippet', default: '—', description: 'Optional override display content (defaults to the shortcut text).' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Shortcut</h1>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[10rem] flex-wrap items-center justify-center gap-4 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<Shortcut shortcut="K">⌘K</Shortcut>
				<Shortcut shortcut="/">/</Shortcut>
				<Shortcut shortcut="enter">↵</Shortcut>
				<Shortcut shortcut="esc">esc</Shortcut>
			</div>
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Shortcut shortcut="K">⌘K</Shortcut>
<Shortcut shortcut="/">/</Shortcut>
<Shortcut shortcut="enter">↵</Shortcut>`, "svelte")}</code></pre>
		</div>
	</div>
</section>
