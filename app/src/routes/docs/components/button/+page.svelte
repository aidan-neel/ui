		...
		>
			...
			<div class="flex flex-col divide-y divide-border/60">
				<div class="flex flex-col gap-2 px-6 py-4">
					<label
						for="pg-label"
						class="text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted"
					>
						Label
					</label>
					<input
			...
			>
				<span class="text-[0.66rem] font-medium uppercase tracking-wide text-foreground-muted">
					Snippet
				</span>
				<button
					type="button"
...
<div class="flex flex-col gap-16 pt-16">
		...
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
			{#each [{ icon: Layers, title: '10 variants', body: 'Semantic + status + neutral.' }, { icon: Type, title: '4 sizes', body: 'Token-driven height + padding.' }, { icon: Link, title: 'Polymorphic', body: 'Pass `href` to render an anchor.' }] as card}
				<div class="flex flex-col gap-2 rounded-[var(--radius-lg)] border border-border bg-card p-4">
					...
					<p
						class="m-0 text-[1rem] font-medium tracking-tight"
						style="font-family: var(--font-header);"
					>
						{card.title}
					</p>
				...
				>
					...
					<div class="flex flex-col gap-1 border-t border-border/70 px-4 py-3">
						<div class="flex items-center justify-between gap-2">
							<p class="m-0 text-[0.86rem] font-semibold">{v.label}</p>
							<button
								type="button"
						...
						<p class="m-0 text-[0.72rem] font-medium text-foreground-muted">{v.tone}</p>
			...
			<div class="grid grid-cols-1 divide-y divide-border/60 sm:grid-cols-2 lg:grid-cols-4 lg:divide-x lg:divide-y-0">
				{#each sizeList as s}
					<div class="flex flex-col gap-1 p-4">
						<p class="m-0 font-mono text-[0.78rem] font-semibold">{s.label}</p>
		...
		<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
			{#each [{ label: 'Resting', node: 'rest' }, { label: 'Hover', node: 'hover' }, { label: 'Focus', node: 'focus' }, { label: 'Active', node: 'active' }, { label: 'Disabled', node: 'disabled' }] as s}
				...
				>
					...
					<span class="text-[0.74rem] font-medium text-foreground-muted">{s.label}</span>
				</div>
			{/each}
		</div>
	</section>
  ...
  <Button variant="ghost"><Trash size={14} /></Button>
	...
	<section id="api" class="scroll-mt-20 flex flex-col gap-5">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card">
			<div
				class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 border-b border-border bg-secondary/40 px-4 py-2.5 text-[0.7rem] font-medium uppercase tracking-wide text-foreground-muted max-md:hidden"
			>
				<span>Prop</span>
				<span>Type</span>
				...
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.8fr_0.5fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex items-center gap-2">
							...
							>
								<code class="font-mono text-[0.82rem] font-semibold text-foreground">
									{row.prop}
								</code>
