<script lang="ts">
	...
	let pgDescription = $state('Roughly one email per month — opt out anytime.');

	const apiRows = [
		{ prop: 'checked', type: 'boolean', default: 'false', description: 'Bindable. Two-way with the user via `bind:checked`.' },
		{ prop: 'label', type: 'string', default: '—', description: 'Short label rendered to the right of the box.' },
		{ prop: 'description', type: 'string', default: '—', description: 'Optional second line in foreground-muted.' },
		{ prop: 'disabled', type: 'boolean', default: 'false', description: 'Greys out + blocks pointer/keyboard activation.' },
		{ prop: 'class', type: 'string', default: '—', description: 'Tailwind classes appended via cn().' }
	];

	const playgroundCode = $derived(`<Checkbox
  bind:checked={value}
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Checkbox</h1>
	...
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="grid min-h-[10rem] place-items-center border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
				<Checkbox variant="default" bind:checked={pgChecked} label={pgLabel || 'Label'} description={pgDescription} />
			</div>
			...
			<div class="flex flex-col divide-y divide-border/60">
				...
				<div class="flex flex-col gap-2 px-6 py-4">
					...
					<input id="pg-desc" bind:value={pgDescription} class="h-9 w-full rounded-[var(--radius-md)] border border-border bg-[var(--color-field)] px-3 text-[0.86rem] text-foreground outline-none transition-[border-color,box-shadow] focus:border-[var(--field-focus-border)] focus:shadow-[0_0_0_3px_var(--color-ring)]" />
				</div>
			</div>
	...
	<section class="scroll-mt-20 flex flex-col gap-5">
		<div class="flex flex-col gap-1">
			<div class="flex items-center gap-2">
				...
				<h2 class="m-0 text-[1.4rem] font-medium tracking-tight" style="font-family: var(--font-header);">States</h2>
			</div>
		</div>
			...
			{#each [{ label: 'Unchecked', value: false }, { label: 'Checked', value: true }, { label: 'Disabled', value: false, disabled: true }, { label: 'Disabled + on', value: true, disabled: true }] as state}
				<div class="flex flex-col gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-4">
					<div class="grid min-h-[3.5rem] place-items-center">
						<Checkbox variant="default" checked={state.value} disabled={state.disabled} label={state.label} />
					</div>
				</div>
			{/each}
		</div>
