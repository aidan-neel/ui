<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'placement', type: '"top" | "right" | "bottom" | "left"', default: '"bottom"', description: 'Side the floating panel prefers.' },
		{ component: 'Trigger', prop: '…ButtonProps', type: '—', default: '—', description: 'Renders as a Button. Inherits Button props.' },
		{ component: 'Content', prop: 'children', type: 'Snippet', default: '—', description: 'Floating surface positioned via Floating UI.' },
		{ component: 'Content', prop: 'class', type: 'string', default: '—', description: 'Wraps the inner panel — set width with `w-[N]`.' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Popover</h1>
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			The primitive behind every floating menu in Silk. Pair a trigger with an anchored surface
			that handles outside-click, escape, focus, and collision-aware placement for you.
		</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
				...
				<Popover.Root placement="bottom">
					...
					<Popover.Content class="w-72">
						<div class="flex flex-col gap-2 p-3">
							<p class="m-0 text-[0.86rem] font-semibold">Notifications</p>
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Popover.Root placement="bottom">
  <Popover.Trigger variant="outlined">
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Popover.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
