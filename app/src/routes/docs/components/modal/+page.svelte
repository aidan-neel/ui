<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'open', type: 'boolean', default: 'false', description: 'Required prop. Bindable for controlled state.' },
		{ component: 'Trigger', prop: '…ButtonProps', type: '—', default: '—', description: 'Renders as a Button.' },
		{ component: 'Content', prop: 'allowClickOutside', type: 'boolean', default: 'true', description: 'Disable to force the user to confirm or close explicitly.' },
		{ component: 'Content', prop: 'role', type: '"dialog" | "alertdialog"', default: '"dialog"', description: 'ARIA role.' },
		{ component: 'Title / Description / Header / Footer', prop: '—', type: '—', default: '—', description: 'Layout slots.' },
		{ component: 'Close', prop: 'onclick', type: '() => void', default: '—', description: 'Closes the modal.' },
		{ component: 'Confirm', prop: 'onclick', type: '() => void', default: '—', description: 'Fires action then closes.' }
	];
...
<svelte:head>
	...
	<meta name="description" content="The primitive overlay shared by Dialog, AlertDialog, and Sheet." />
</svelte:head>
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Modal</h1>
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			The low-level overlay primitive used by Dialog, AlertDialog, and the publish/admin flows.
			You probably want one of those — reach for Modal directly only when you need a custom
			surface shape.
		</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Modal.Root open={false}>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Modal.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
