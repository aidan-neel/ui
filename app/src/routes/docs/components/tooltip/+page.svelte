<script lang="ts">
	...
	const apiRows = [
		{ component: 'Root', prop: 'placement', type: '"top" | "right" | "bottom" | "left"', default: '"top"', description: 'Preferred side.' },
		{ component: 'Root', prop: 'delay', type: 'number', default: '120', description: 'Time in ms before the tooltip opens on hover.' },
		{ component: 'Root', prop: 'closeDelay', type: 'number', default: '70', description: 'Grace period before closing — lets the user move into the tooltip.' },
		{ component: 'Trigger', prop: 'children', type: 'Snippet', default: '—', description: 'The element the tooltip describes. Wrap a Button/element, do not pass Button props here.' },
		{ component: 'Content', prop: 'children', type: 'Snippet', default: '—', description: 'Tooltip body — keep it to a sentence.' }
	];
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Tooltip</h1>
		<p class="m-0 max-w-[42rem] text-[1rem] leading-relaxed text-foreground-muted">
			A one-line hover hint for icons, abbreviations, and dense affordances. Mobile users won't
			see it — never put critical info behind a tooltip.
		</p>
	</div>
...
<section class="pt-10">
	<div class="relative">
		...
		<div class="overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-[var(--shadow-sm)]">
			<div class="flex min-h-[12rem] flex-wrap items-center justify-center gap-3 border-b border-border/70 bg-[linear-gradient(135deg,color-mix(in_srgb,var(--color-secondary)_60%,transparent),transparent_70%)] p-8">
			...
			<pre class="m-0 overflow-x-auto bg-secondary/40 px-6 py-4 font-mono text-[0.78rem] leading-relaxed text-foreground"><code>{@html highlight(`<Tooltip.Root placement="top">
  <Tooltip.Trigger>
    <Button variant="outlined">
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Tooltip.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
