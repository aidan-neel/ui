<script lang="ts">
	...
	import { Badge as BadgeComp } from '$lib/silk/components/badge';
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1 class="m-0 text-[2.6rem] font-medium leading-[1] tracking-[-0.035em] md:text-[3rem]" style="font-family: var(--font-header);">Tabs</h1>
...
<section class="pt-12 flex flex-col gap-5">
	<div class="flex flex-col gap-1">
		<div class="flex items-center gap-2">
			...
			<h2
				class="m-0 text-[1.4rem] font-medium tracking-tight"
	...
	<div class="flex flex-col gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-5">
		<div class="flex items-center justify-between gap-3 max-sm:flex-col max-sm:items-start">
			<div>
				<p class="m-0 text-[0.78rem] font-medium text-foreground-muted">Project activity</p>
				<p
				...
				<div class="grid grid-cols-3 gap-3 max-sm:grid-cols-1">
					{#each [{ label: 'Open issues', value: '23', tone: 'text-[var(--color-warning)]' }, { label: 'Merged this week', value: '47', tone: 'text-[var(--color-success)]' }, { label: 'Build status', value: 'Passing', tone: 'text-[var(--color-success)]' }] as stat}
						<div class="rounded-[var(--radius-md)] border border-border bg-background/40 p-3">
							...
							<p class="m-0 mt-1 text-[1.4rem] font-semibold tracking-tight {stat.tone}">
								{stat.value}
							</p>
						</div>
					{/each}
	...
	<div class="grid grid-cols-1 gap-3 md:grid-cols-2">
		<div class="flex flex-col gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-5">
			<p class="m-0 text-[0.78rem] font-medium text-foreground-muted">Billing period</p>
			...
			<div class="flex items-baseline gap-1.5">
				<span
					class="text-[2.4rem] font-medium tracking-tight"
					style="font-family: var(--font-header);"
				>
		...
		<!-- Settings -->
		<div class="flex flex-col gap-3 rounded-[var(--radius-lg)] border border-border bg-card p-5">
			<p class="m-0 text-[0.78rem] font-medium text-foreground-muted">Settings</p>
			...
			<ul class="flex flex-col divide-y divide-border/60">
				{#each apiRows as row}
					<li class="grid grid-cols-[1fr_1.4fr_0.6fr] gap-3 px-4 py-3 max-md:grid-cols-1">
						<div class="flex flex-col gap-1"><code class="font-mono text-[0.7rem] text-foreground-muted">Tabs.{row.component}</code><code class="font-mono text-[0.82rem] font-semibold">{row.prop}</code></div>
