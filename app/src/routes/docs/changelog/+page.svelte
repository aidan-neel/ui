	...
	const releases: Release[] = [
		{
			...
			entries: [
				...
				},
				{
					tag: 'feature',
					icon: Palette,
					title: 'Theme registry + admin gating',
					body: 'A new Elysia + Prisma + Postgres service for distributing themes. JWT-gated admin endpoints (httpOnly cookie, HMAC-SHA256), available only in dev mode.'
				},
				{
					tag: 'polish',
					icon: Sparkles,
...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1
			class="m-0 max-w-[26ch] text-[2.6rem] font-medium leading-[1.02] tracking-[-0.04em] md:text-[3rem]"
	...
	{#each releases as release, idx}
		<article class="relative flex flex-col gap-5">
			...
			<div class="flex flex-col gap-2">
				<div class="flex flex-wrap items-center gap-2">
					<span
						class="inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-2 py-0.5 font-mono text-[0.74rem] font-semibold text-foreground"
					>
				...
				<h2
					class="m-0 text-[1.5rem] font-medium leading-tight tracking-[-0.025em]"
					style="font-family: var(--font-header);"
				>
					{release.title}
				</h2>
				...
				{#each release.entries as entry}
					...
					>
						<div class="flex items-center justify-between gap-2">
							...
							<span
								class={`inline-flex items-center rounded-md border px-2 py-0.5 text-[0.62rem] font-medium uppercase tracking-wide ${tagStyles[entry.tag].cls}`}
							>
								{tagStyles[entry.tag].label}
							</span>
						</div>
						<p
							class="m-0 text-[0.96rem] font-medium tracking-tight"
							style="font-family: var(--font-header);"
						>
							{entry.title}
						</p>
