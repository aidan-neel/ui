...
<header class="flex flex-col gap-5 border-b border-border/60 pb-10">
	...
	<div class="flex flex-col gap-3">
		<h1
			class="m-0 text-[2.6rem] font-medium leading-[1.02] tracking-[-0.04em] md:text-[3rem]"
	...
	{#each steps as step}
		...
		>
			<div class="flex shrink-0 items-start gap-3 md:w-48">
				<span
					class="grid size-8 shrink-0 place-items-center rounded-full bg-primary/12 font-mono text-[0.8rem] font-semibold text-primary"
				>
					{step.id}
				</span>
				<p
					class="m-0 pt-1 text-[0.94rem] font-medium leading-tight tracking-tight"
					style="font-family: var(--font-header);"
				>
					{step.title}
				</p>
