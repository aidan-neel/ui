 ...
 *           wrapper. The public Root/Trigger/Content/Title/Description
 *           API and the `openDelay`/`closeDelay` props on Root are
 *           preserved. The trigger renders as `<a>` when `href` is
 *           passed (anchor mode) and `<span>` otherwise — both shapes
 *           unchanged from 1.0.0.
 *
 *           Removed: the `HoverCardState` type export (was used only
 *           by the standalone implementation; no external consumers).
 *
