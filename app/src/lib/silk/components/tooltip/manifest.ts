 ...
 *
 * Version history:
 *   1.0.0 — implicit pre-Phase-4 standalone implementation.
 *   2.0.0 — Phase 4 item 7 (F-29 collapse): rewritten as a popover
 *           wrapper. The public Root/Trigger/Content API and the
 *           `delay`/`closeDelay`/`placement` props on Root are
 *           preserved. The trigger is a `<span>` (was `<span>` before;
 *           the DOM shape is unchanged). The Content's `role="tooltip"`
 *           commitment is preserved.
 *
 *           Breaking change: the deprecated TooltipContext context
 *           shape and id-generation are gone — consumers that imported
 *           types from the old API will see compile errors.
 */
