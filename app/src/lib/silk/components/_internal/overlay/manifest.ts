 ...
 *
 * Internal visibility per pattern guide §2.5 — the CLI installs this as a
 * transitive dependency only. `npx silk add overlay` is not a valid
 * command; consumers reach overlay only through modal or sheet.
 *
 * Version history:
 *   1.0.0 — Phase 4 item 6 initial release. Extracted from the duplicated
 *           focus-trap / click-outside / Escape / body-overflow logic that
 *           lived in modal-content.svelte and sheet-content.svelte
 *           (resolves F-30 from the Phase 1 audit).
 */
